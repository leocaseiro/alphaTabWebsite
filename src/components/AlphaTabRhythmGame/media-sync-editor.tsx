...
const useMediaElementPlaybackTime = (
  ...
  const [mediaElement, setMediaElement] = useState<HTMLMediaElementLike | null>(
    null,
  );
    ...
    switch (api.actualPlayerMode) {
      case alphaTab.PlayerMode.EnabledBackingTrack:
        setMediaElement(
          (
            api.player!
              .output as alphaTab.synth.IAudioElementBackingTrackSynthOutput
          )?.audioElement ?? null,
        );
        break;
...
export const MediaSyncEditor: React.FC<MediaSyncEditorProps> = ({
  ...
  const [playbackTime, setPlaybackTime] = useMediaElementPlaybackTime(
    api,
    youtubePlayer,
  );
  ...
  useEffect(() => {
    if (
      syncArea.current &&
      (lastScroll.current === 0 || performance.now() - lastScroll.current > 500)
    ) {
      const xPos = timePositionToX(
      ...
      // is out of screen?
      if (
        xPos < scrollOffset + threshold ||
        xPos - scrollOffset > canvasWidth - threshold
      ) {
  ...
  const newYoutubeUrlError = extractYouTubeVideoId(newYoutubeUrl)
    ? ""
  ...
  useEffect(() => {
    if (syncPointInfo) {
      setVirtualWidth(
        (s) => pixelPerMilliseconds * syncPointInfo.endTime * zoom,
      );
    }
  }, [syncPointInfo, zoom]);
    ...
    input.onchange = () => {
      if (input.files?.length === 1) {
        ...
        reader.onload = (e) => {
          ...
          score.backingTrack.rawAudioFile = new Uint8Array(
      ...
      {isYoutubeModalOpen && (
        <div className={`${styles.modal} ${styles["modal-sm"]}`}>
          <div className={styles["modal-body"]}>
            ...
            <p>
              Enabling the synchronization with YouTube, requires the use of
              Youtube APIs loaded from youtube.com and youtube-nocookie.com.
              With pressing <strong>Load</strong> you accept the load of these
              resources and accept related policies from Google and Youtube.
              <ul>
                <li>
              ...
              It should be obvious that syncing with YouTube requires this, but
              we still want to inform you of the impact.
            </p>

            <div className={styles["modal-actions"]}>
              <div
                data-tooltip-id="tooltip-playground"
                data-tooltip-content={newYoutubeUrlError}
              >
                <button
                  type="button"
      ...
      {isGenerateCodeModalOpen && (
        <div className={`${styles.modal} ${styles["modal-lg"]}`}>
          <div className={styles["modal-body"]}>
            ...
            The following code helps you setting up the sync points in your app.
            The exact logic depends on your app, but generally you would store
            the sync points in parallel to the file (e.g. in a database) and
            load them along the main file. Then when the file is loaded by
            alphaTab you additionally apply the sync points.
            <Tabs
              defaultValue="ts"
              values={[
            ...
            >
              ...
              <TabItem value="at">
                Place the following alphaTex at the end of your alphaTex song
                for applying the sync points on load.
                <CodeBlock title="alphaTex Sync Points">
                  {[".", syncPointsToAlphaTex(syncPointInfo)].join("\n")}
                </CodeBlock>
              </TabItem>
            </Tabs>
            <div className={styles["modal-actions"]}>
      ...
      <div className={styles.toolbar}>
        ...
        <div>
          {mediaType.type === MediaType.Audio && (
            ...
            >
              <FontAwesomeIcon icon={solid.faDownload} /> Download Synced GP
              File
            </button>
          )}
